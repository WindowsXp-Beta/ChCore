...
int read_whole_file(int fd, char *buf)
{
        ...
        do {
                struct ipc_msg *ipc_msg =
                        ipc_create_msg(fs_ipc_struct_for_shell,
...
int do_complement(char *buf, char *complement, int complement_time)
{
        ...
        ret = open_file("/", fd);
        if (ret < 0) {
                chcore_bug("chcore open file wrong");
        }
...
void print_file_content(char *path)
{
        ...
        if (ret < 0) {
                chcore_bug("open file wrong");
        }
        ret = read_whole_file(fd, buf);
        printf("%s", buf);
        ret = close_file(fd);
        if (ret < 0) {
                chcore_bug("close file wrong");
        }
        printf("%s", buf);
        /* LAB 5 TODO END */
}
...
void fs_scan(char *path)
{
        ...
        ret = open_file(path, fd);
        if (ret < 0) {
                chcore_bug("open directory wrong");
        }
        demo_getdents(fd);
        // ret = close_file(fd);
        // if (ret < 0) {
        // chcore_bug("close directory wrong");
        // }
        /* LAB 5 TODO END */
}
...
int run_cmd(char *cmdline)
{
        ...
        strcat(pathbuf, cmdline);
        int ret = chcore_procm_spawn(pathbuf, caps);
        /* LAB 5 TODO END */
        return ret;
}
